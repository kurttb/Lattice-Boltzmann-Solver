// Example case using the modularized library for the case of a cavity with two moving lids
#include <string>
#include <cmath>
...
int main() {
	...
	std::string BCBottomType = "WallTangentVelocity"; // Bounce-back (equivalent to no-slip)
	double uT2 = uChar / 2;
	prob.setBC(BCBottom, BCBottomType, uT2); // Set BC

	std::string BCLeft = "Left";
	std::string BCLeftType = "BounceBack";
	prob.setBC(BCLeft, BCLeftType);

	std::string BCRight = "Right";
	std::string BCRightType = "BounceBack";
	...
	// Write the output
	std::string filePath = "DoubleLDC.vtk";
